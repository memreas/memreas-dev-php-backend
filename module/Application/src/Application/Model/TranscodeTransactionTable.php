...
class TranscodeTransactionTable
{
    ...
    public function getTranscodeTransaction ($transcode_transaction_id)
    {
        ...
        if (! $row) {
            throw new \Exception(
                    "Could not find row transcode_transaction_id::$transcode_transaction_id");
        }
        return $row;
    }

    public function saveTranscodeTransaction (
            TranscodeTransaction $transcode_transaction)
    {
        $data = array(
        ...
        try {
            if (isset($transcode_transaction->transcode_transaction_id)) {
                if ($this->getTranscodeTransaction(
