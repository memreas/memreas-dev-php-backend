...
class MemreasTranscoder
{
            ...
            $tns_sized = array(
                    ...
                    "79x80" => $this->resizeImage(
                            $this->homeDir . self::DESTDIR . self::THUMBNAILSDIR .
                                     self::_79X80, $filename, 
                                    basename($filename), 79, 80),
                    "448x306" => $this->resizeImage(
                            $this->homeDir . self::DESTDIR . self::THUMBNAILSDIR .
                             self::_448X306, $filename, basename($filename), 448, 
                            306),
                    "384x216" => $this->resizeImage(
                            $this->homeDir . self::DESTDIR . self::THUMBNAILSDIR .
                             self::_384X216, $filename, basename($filename), 384, 
                            216),
                    "98x78" => $this->resizeImage(
                            $this->homeDir . self::DESTDIR . self::THUMBNAILSDIR .
                             self::_98X78, $filename, basename($filename), 98, 
                            78),
                    "1280x720" => $this->resizeImage(
                            $this->homeDir . self::DESTDIR . self::THUMBNAILSDIR .
                             self::_1280x720, $filename, basename($filename), 
                            1280, 720)
            );
    ...
    public function transcode ($type)
    {
        ...
        // $command =array_merge(array( '-i',$this->destRandMediaName,'-vcodec',
        ...
        if ($type == 'web') {
            ...
            // $ffmpeg_logger
            // . '2>&1';
            // $cmd = 'nice ' . $this->ffmpegcmd ." -i $this->destRandMediaName
            // $qv $transcoded_file ".'2> $ffmpeg_logger';
        } else 
            if ($type == '1080p') {
                
                $qv = ' -c:v libx265 -preset ' . $this->compression_preset_1080p .
                         ' -x265-params crf=28 -c:a aac -strict experimental -b:a 128k ';
                // $qv = ' -c:v libx265 -c:a libfdk_aac -preset medium
                // -profile:v main -level 4.0 -movflags +faststart -pix_fmt
                // yuv420p -b:a 240k ';
                // $qv=' -c:v libx264 -c:a libfdk_aac -preset fast -profile:v
                ...
                if ($type == 'hls') {
                    ...
                    Mlog::addone(__CLASS__ . __METHOD__ . '$transcoded_file', 
                            $transcoded_file);
                    Mlog::addone(__CLASS__ . __METHOD__ . '$transcoded_file', 
                    ...
                    // $transcoded_mp4_file .
                    // " -c:v libx265 -preset medium -x265-params crf=28 -c:a
                    ...
                    if ($type == 'audio') {
                        ...
                        $transcoded_file = $this->homeDir . self::CONVDIR .
                                 self::AUDIODIR . $this->MediaFileName . $aacext;
        ...
        if ($type == "hls") {
            $s3file = $this->s3prefixpath . $type . '/' . $this->MediaFileName .
                     '.m3u8';
            Mlog::addone(
            ...
            foreach (glob($pat) as $filename) {
                ...
                Mlog::addone(
                        __CLASS__ . __METHOD__ .
                                 '$this->aws_manager_receiver->pushMediaToS3(...)', 
                                MemreasConstants::S3HLSBUCKET);
                $this->aws_manager_receiver->pushMediaToS3($filename, $s3tsfile, 
                        "video/mp2t", true, MemreasConstants::S3HLSBUCKET);
            }
        } else 
            if ($this->is_audio) {
                $this->aws_manager_receiver->pushMediaToS3($transcoded_file, 
        ...
        Mlog::addone(__CLASS__ . __METHOD__ . '::complete::transcode_status', 
    ...
    {
        $duration_in_minutes = $this->duration / 60; // duration stored in db in
                                                     // seconds
