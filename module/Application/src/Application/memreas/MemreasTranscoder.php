	...
	public function transcode() {
		try {
			...
			if ($this->type == 'copyright') {
				...
				// Testing -tune grain
				$qv = ' -c:v libx264 ' . ' -preset ' . $this->compression_preset_web . ' -crf 18 ' . ' -tune film ' . ' -pix_fmt yuv420p ' . ' -movflags ' . ' +faststart ' . ' -c:a aac ' . ' -strict experimental ' . '-b:a 128k ';
				...
				// Testing remove bit rate and profile...
				$cmd = 'nice -' . $this->nice_priority . ' ' . $this->ffmpegcmd . " -nostats -re -y -i " . $input_file . ' -hls_list_size 0 -hls_allow_cache 0 -hls_segment_filename ' . $transcoded_hls_ts_file . "%05d.ts " . $transcoded_file;
