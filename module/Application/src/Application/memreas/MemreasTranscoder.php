	...
	public function transcode() {
		try {
			...
			if ($this->type == 'copyright') {
				...
				// Testing -tune grain
				$qv = ' -c:v libx264 ' . ' -profile:v high -level 4.2 ' . ' -preset ' . $this->compression_preset_web . ' -crf 18 ' . ' - tune grain ' . ' -pix_fmt yuv420p ' . ' -movflags ' . ' +faststart ' . ' -c:a aac ' . ' -strict experimental ' . '-b:a 128k ';
