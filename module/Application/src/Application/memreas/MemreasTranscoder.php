	...
	public function markMediaForTranscoding($message_data) {
		try {
			...
			if (isset ( $message_data ['is_video'] ) && ($message_data ['is_video'] == 1)) {
				...
				Mlog::addone ( __CLASS__ . __METHOD__ . __LINE__ . 'is_audio', $message_data ['is_audio'] );
				$message_data ['is_image'] = 0;
		...
		try {
			...
			if (isset ( $message_data )) {
				...
				if ($this->is_video) {
					...
					// Image section
					$this->transcode_job_meta = array ();
					$this->transcode_job_meta = $this->createThumbNails ( $this->is_image );
					$this->memreas_media_metadata ['S3_files'] ['transcode_progress'] [] = 'thumbnails_complete';
					// update media metadata and transcode transaction
					// metadata
					$this->persistMedia ();
			...
			try {
				...
				$this->aws_manager_receiver->sesEmailErrorToAdmin ( json_encode ( $error_data, JSON_PRETTY_PRINT ) );
	...
	public function transcode($type) {
		try {
			...
			if ($type == 'web') {
				 ...
				 */
				$qv = ' -c:v libx265 -preset ' . $this->compression_preset_web . ' -x265-params crf=28 -c:a aac -strict experimental -b:a 128k ';
				...
				$qv = ' -c:v libx265 -preset ' . $this->compression_preset_1080p . ' -x265-params crf=28 -c:a aac -strict experimental -b:a 128k ';
				...
				// above
				$transcoded_mp4_file = $this->homeDir . self::CONVDIR . self::WEBDIR . $this->MediaFileName . $mpeg4ext;
				...
				// Old h264 impl
				$cmd = 'nice -' . $this->nice_priority . ' ' . $this->ffmpegcmd . " -re -y -i " . $transcoded_mp4_file . " -map 0 " . " -pix_fmt yuv420p " . " -vcodec libx264 " . " -acodec libfdk_aac " . " -r 25 " . " -profile:v main -level 4.0 " . " -b:v 1500k " . " -maxrate 2000k " . " -force_key_frames 50 " . " -flags -global_header " . " -f segment " . " -segment_list_type m3u8 " . " -segment_list " . $transcoded_file . " -segment_time 10 " . " -segment_format mpeg_ts " . $transcoded_hls_ts_file . "%05d.ts" . ' 2>&1';
